...
class ToolNodeHandler:
    ...
    async def execute(
        ...
        """
        tool_name = config.get("tool_name") or config.get("code")
...
class CouncilNodeHandler:
    ...
    async def execute(
        ...
        # Check for pre-parsed CouncilConfig (from TopologyEngine)
        council_config_obj = c_config.get("council_config")

        prompt = c_config.pop("prompt", "Please analyze.")

        if isinstance(council_config_obj, CouncilConfig):
            council_config = council_config_obj
        else:
            # Fallback for manual dict config (legacy or test usage)
            # Remove keys that are not part of CouncilConfig to prevent extra fields error if strict
            # CouncilConfig has 'strategy' and 'voters'
...
class LLMNodeHandler:
    ...
    async def execute(
        ...
        If council_config is present, delegates to CouncilNodeHandler.
